...
13. **Preservar a voz guardada** — `Guarda.acharFlex(id, i, texto, marca)` acha o trecho pelo **hash do texto**, não só pela posição: vale quando a posição muda (cumprimento ligado/desligado) e quando o **modelo** da mesma voz muda (nunca mistura vozes diferentes). O leitor (`pegarAudio`) e a fila usam isso. `Guarda.guardar` pede `navigator.storage.persist()` na primeira gravação; Ajustes mostra se está protegida. **Nunca mudar o formato da chave** (`id|índice|motor|voz|modelo|hash`) nem o `hashTexto`: é o que liga o áudio guardado ao texto. Mudar o tamanho dos trechos (`tetoDoPedaco`) muda os textos dos trechos e faz gerar de novo — evitar.
    Arquivo de voz (`.chvoz`): `CHVOZ1` + tamanho do índice (uint32, little-endian) + índice JSON `{v, app, em, itens:[{k,t,n}], completos}` + os áudios na ordem do índice. Importar nunca troca um trecho que já existe.
14. **Fila de voz** — módulo `FilaVoz`, estado em `ch_fila_voz` (`ids`, `ligada`, `parou`). Uma história por vez, um trecho por vez, com os trechos de `Leitor.trechos(h)` (a mesma divisão do leitor, função `filaDe`). Erro de cota/rede/servidor: espera 30 s, 1, 2, 5 e 10 min e tenta de novo sem limite; sem chave, chave recusada (400/401/403/404) ou sem voz: pausa e mostra o motivo. Espera enquanto o leitor está aberto. Segura a tela acesa (Wake Lock) enquanto trabalha. Continua sozinha ao abrir o app.
15. **Versão** — `HISTORICO` (a primeira linha é a versão atual), `telaNovidades`, `telaAviso`.
